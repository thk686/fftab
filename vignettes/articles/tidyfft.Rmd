...
Unlike R's native `fft` function, `tidy_fft` outputs both the fourier coefficients and
...
Currently, the `tidy_fft` function supports numeric and complex vectors, time series
objects, and multidimensional arrays. I plan to extend this to the multivariate case,
in which multiple columns in a matrix or table are transformed simultaneously.
...
```{r}
v_tfft |> to_rect() |> print(n = 3)
v_tfft |> to_polr() |> print(n = 3)
```
...
`tidy_fft` package functions produce the same output given any input representation. You do not
...
s1 <- seq(-pi, pi, length.out = 200) |>
  tibble::as_tibble() |>
  dplyr::mutate(x = value, y = cos(8 * value), treatment = "original", .keep = "none")
ggplot(s1) +
...
```{r fig.asp=0.5}
s1_tfft <- with(s1, y |> tidy_fft() |> to_polr())
plot(s1_tfft) # calls ggplot internally
```
...
```{r fig.asp=0.5}
s1 |>
  dplyr::mutate(y = s2_tfft |> tidy_ifft(), treatment = "shifted") |>
  dplyr::bind_rows(s1) |>
